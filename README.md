...
To configure the JavaScript module, you can pass a config object to `seon.init(config)` and/or `seon.getSession(config)`. These are two separate config objects, and none of the parameters are required.
...
| portScan                   |                             | Settings for the local port scanning                                                                                                                                                                                                                            |                | _Details below_. Available since 6.12.0                                                                                                                                                                                          |
|                             | enabled                     | Whether to start the local port scanning already during `init()`, instead of waiting for the `getSession()` call.                                                                                                                                              | `false`        | Port scanning takes some time to complete, so starting it earlier during `init()` gives it more time to finish before `getSession()` is called, resulting in a more accurate result. If enabled, the scan started during `init()` is reused by `getSession()` regardless of its `silentMode` option. If left disabled, port scanning still runs during `getSession()` as long as `silentMode` is off. |
|                             | force                       | Whether to force a fresh port scan when `init()` is called, even if one is already running.                                                                                                                                                                     | `false`        | By default a port scan started during a previous `init()` call is kept running and reused, so repeated `init()` calls do not restart it. Enable this to abort the running scan and start a new one instead. Only considered when `enabled` is true.                                                     |

#### getSession() configuration
...
|                                           | enabled                     | Enables geolocation collection in the browser.                                                                                                                                                                                                                 | `false`                                                                             |                                                                                                                                                                                                                                  |
...
| referrer                                  |                             | Configures the `referrer` field in the response                                                                                                                                                                                                                |                                                                                     | _Details below_                                                                                                                                                                                                                  |
...
| silentMode                                |                             | Stops the SDK from triggering warnings & errors on the DevTools console.                                                                                                                                                                                            | `true`                                                                              | Turning this off will allow the SDK to enable additional features.                                                                                                                                                               |
...
| windowLocation                            |                             | Configures the `windowLocation` field in the response                                                                                                                                                                                                          |                                                                                     | _Details below_                                                                                                                                                                                                                  |
|                                           | maxLength                   | Maximum length of the URL                                                                                                                                                                                                                                      | `128`                                                                               | Allowed range: 0-1024                                                                                                                                                                                                            |
|                                           | searchParams                | Whether to include search parameters of the URL                                                                                                                                                                                                                | `false`                                                                             |                                                                                                                                                                                                                                  |
...
SEON JavaScript library collects device information and prepares a payload to use in Fraud API. The information on client side is encrypted, the data will be available in the Fraud API response and on the Admin Panel. Some fields can be `null`, if the actual browser does not support or return data for that specific data point. In every other case, data types are preserved.
...
- Introduce `customCookieEndpoint` configuration option for first-party HTTP cookie support
- Introduce `performanceMode` configuration option which speeds up fingerprinting time but reduces accuracy
...
- Fixed an issue where the `cookie_hash` changed between fingerprinting attempts in some rare cases
- Added `experimentalHardwareAccelerationDetection` config option
...
- #### The `browser_hash` field is calculated differently, resulting in different values for a given device. This means these values are going to break between versions.
